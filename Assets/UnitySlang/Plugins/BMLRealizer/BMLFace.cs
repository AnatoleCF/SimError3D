...
namespace BMLRealizer
{
    ...
    public class BMLFace : BMLBehavior
    {
        ...
        public override void Parse(XmlReader reader)
        {
            ...
            type = TryParseAtribute<string>(reader, "type", "none", false);
            side = TryParseAtribute<string>(reader, "side", "none", false);
            amount = TryParseAtribute<float>(reader, "amount", 0.5f, false);
            au = TryParseAtribute<string>(reader, "au", "none", false);
            interpolate = TryParseAtribute<float>(reader, "interpolate", 0, false);
            ...
            if(reader.ReadToFollowing("amount")) {
                ...
                amount = float.Parse(tmp);
            }
            if(reader.ReadToFollowing("au")) {
                string tmp = reader.ReadElementContentAsString();
                au = tmp.Replace("\"","");
            }
        }
